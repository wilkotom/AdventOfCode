...
fn main() {
    ...
    while !signal {
        candidate += 1;
        print!("{} ", candidate);
        let response = run_program(&mut instructions, candidate);
        signal = response.1;
    }
    println!("{}", candidate)
...
fn read_instructions(filename: &str) -> Vec<AssemBunnyInstruction> {
    ...
    for line in raw_data.split('\n') {
        ...
        let instruction = match words[0] {
            ...
            "out" => {
                AssemBunnyInstruction::Output(str_to_value(words[1]))
            }
            _ => unimplemented!()
        };
            ...
            AssemBunnyInstruction::Toggle(value) => {
                ...
                if location_to_flip >= 0 && location_to_flip < program.len()  as i32 {
                    program[location_to_flip as usize] = match program.get(location_to_flip as usize).unwrap() {
                        ...
                        AssemBunnyInstruction::Output(x) => AssemBunnyInstruction::Increment(*x)
                    };
                }
                program_counter += 1;
            },
            AssemBunnyInstruction::Output(value) => {
                let out = match value {
                    Value::Literal(v) => *v,
                    Value::RegisterValue(r) => *registers.get(r).unwrap_or(&0)
                };
